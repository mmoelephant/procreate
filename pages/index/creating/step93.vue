...
import { formValidate931 } from '../../../plugins/formValidate931'
import { formValidate103 } from '../../../plugins/formValidate103'
import shu from '../../declaration/great/cover3'
export default {
  components: {
    shu
  },
  ...
  methods: {
    ...
    next() {
      ...
      if (!formValidate931(this.form, this)) return
      if (!formValidate103(this.form, this)) return
      this.form.files = deepCopy(this.files1)
      this.loading = true
      const commondata = JSON.parse(localStorage.getItem('commondata'))
      const data1 = {}
      ...
      data1.files = JSON.stringify(this.files1)
      data2 = datawork(data1)
      this.$api.commit_create3(data2).then((v) => {
        if (v.data.errcode === 0) {
          this.loading = false
          if (this.$route.query.id) {
