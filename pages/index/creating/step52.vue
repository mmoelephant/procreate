<template>
  <div v-loading.fullscreen="loading" class="indexpage">
    <div class="contentbox">
      <div class="contenthead">5.项目主要实施内容</div>
      <div class="content">
        <div>
          <p class="contentname">项目示范内容</p>
          <el-input
            v-model="form.d_nr"
            type="textarea"
            rows="6"
            show-word-limit
            maxlength="500"
            placeholder="请填写项目示范内容"
          >
          </el-input>
        </div>
        <div>
          <p class="contentname">关键问题和难点分析</p>
          <el-input
            v-model="form.d_gj"
            type="textarea"
            rows="6"
            show-word-limit
            maxlength="500"
            placeholder="请填写关键问题和难点分析"
          >
          </el-input>
        </div>
        <div>
          <p class="contentname">考核指标</p>
          <el-input
            v-model="form.d_zb"
            type="textarea"
            rows="6"
...
export default {
  ...
  methods: {
    savemsg() {
      ...
      // data1.category_id = 1
      if (this.form.d_nr && this.form.d_nr.replace(/(^\s*)|(\s*$)/g, '')) {
        data1.d_nr = this.form.d_nr.replace(/(^\s*)|(\s*$)/g, '')
      }
      if (this.form.d_gj && this.form.d_gj.replace(/(^\s*)|(\s*$)/g, '')) {
        data1.d_gj = this.form.d_gj.replace(/(^\s*)|(\s*$)/g, '')
      }
      if (this.form.d_cx && this.form.d_cx.replace(/(^\s*)|(\s*$)/g, '')) {
        data1.d_cx = this.form.d_cx.replace(/(^\s*)|(\s*$)/g, '')
      }
      if (this.form.d_zb && this.form.d_zb.replace(/(^\s*)|(\s*$)/g, '')) {
        data1.d_zb = this.form.d_zb.replace(/(^\s*)|(\s*$)/g, '')
      }
