<template>
  <div v-loading.fullscreen="loading" class="indexpage">
    <div class="contentbox">
      <div class="contenthead">6.技术路线和计划进度</div>
      <div class="content">
        <div>
          <p class="contentname">实施技术路线</p>
          <el-input
            v-model="form.e_js"
            type="textarea"
            rows="6"
            show-word-limit
            maxlength="500"
            placeholder="请填写实施技术路线"
          >
          </el-input>
        </div>
        <div>
          <p class="contentname">工作计划</p>
          <el-input
            v-model="form.e_jh"
            type="textarea"
            rows="6"
...
export default {
  ...
  methods: {
    savemsg() {
      ...
      // data1.category_id = 1
      if (this.form.e_js && this.form.e_js.replace(/(^\s*)|(\s*$)/g, '')) {
        data1.e_js = this.form.e_js.replace(/(^\s*)|(\s*$)/g, '')
      }
      if (this.form.e_jh && this.form.e_jh.replace(/(^\s*)|(\s*$)/g, '')) {
        data1.e_jh = this.form.e_jh.replace(/(^\s*)|(\s*$)/g, '')
      }
      if (this.form.e_zh && this.form.e_zh.replace(/(^\s*)|(\s*$)/g, '')) {
        data1.e_zh = this.form.e_zh.replace(/(^\s*)|(\s*$)/g, '')
      }
      if (this.form.e_tg && this.form.e_tg.replace(/(^\s*)|(\s*$)/g, '')) {
        data1.e_tg = this.form.e_tg.replace(/(^\s*)|(\s*$)/g, '')
      }
