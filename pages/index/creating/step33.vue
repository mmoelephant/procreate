<template>
  <div v-loading.fullscreen="loading" class="indexpage">
    <div class="contentbox">
      <div class="contenthead">3.项目概况及申请理由</div>
      <div class="content">
        <div>
          <p class="contentname">必要性</p>
          <el-input
            v-model="form.d_by"
            type="textarea"
            rows="6"
...
export default {
  ...
  methods: {
    savemsg() {
      ...
      if (this.form.d_by && this.form.d_by.replace(/(^\s*)|(\s*$)/g, '')) {
        data1.d_by = this.form.d_by.replace(/(^\s*)|(\s*$)/g, '')
      }
      if (this.form.d_kx && this.form.d_kx.replace(/(^\s*)|(\s*$)/g, '')) {
        data1.d_kx = this.form.d_kx.replace(/(^\s*)|(\s*$)/g, '')
      }
